...
# Script to redeploy the application with fixed configuration
...
echo "2. Creating a new revision with correct port configuration..."
...
echo "3. Checking new revision details..."
gcloud run services describe $SERVICE_NAME \
  --platform=managed \
  --region=$REGION \
  --project=$PROJECT_ID \
  --format="yaml(status.latestCreatedRevisionName, status.conditions)" 
